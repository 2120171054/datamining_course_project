...
def main():
    ...
    savepath = '/media/mcislab3d/795c8875-2ab9-4936-9a4d-91ccc1301997/mcislab3d/songhao/sunche/model/model/model.ckpt'
    loadpath = '/media/mcislab3d/795c8875-2ab9-4936-9a4d-91ccc1301997/mcislab3d/songhao/sunche/model/model/' 
    fileepoch=open('losssavev7.txt','w')  
    for i in range(0,max_epoch,1):
        init = tf.global_variables_initializer()
        saver = tf.train.Saver()
        session = network.session
        if i!=0:
            if os.path.exists(loadpath + 'checkpoint'):
                saver.restore(session,loadpath + 'model.ckpt')
        ...
        shuffle_array=np.arange(count)
        np.random.shuffle(shuffle_array)
        feature_loadpath='/media/mcislab3d/795c8875-2ab9-4936-9a4d-91ccc1301997/mcislab3d/songhao/sunche/model/train.txt'
        with tf.device("/gpu:0"):
            batch_count = 0;
            loss_total = 0
            data_batch=np.zeros((batch_size,feature_size))
            for p,j in enumerate(shuffle_array):
                theline_data=linecache.getline(feature_loadpath,j+1);
                theline_data.strip()
                ...
                if p%batch_size==batch_size-1:
                    ...
                    _,_loss_=session.run([network.trainer,network.loss],feed_dict={x_input: data_batch,training:True})
                    loss_total += _loss_
                    print ('Rec_Loss1:{} '.format(_loss_))

                    # enc_loss_=session.run([network.enc_trainer,network.rec_loss],feed_dict={network.x_input: data_resnet_batch})
                    # dec_loss_=session.run([network.dec_trainer,network.d_loss,network.rec_loss],feed_dict={network.x_input: data_resnet_batch})
                    # dis_loss_=session.run([network.dis_trainer,network.d_loss],feed_dict={network.x_input: data_resnet_batch})
                    print ('epoch {} have trained line {} of lines {}'.format(i,p,len(shuffle_array)))
            linecache.clearcache()
            if i%10==0:
                saver.save(session, savepath)
                fileepoch.write(str(loss_total/batch_count)+'\n');  
    fileepoch.close()  
         #   else:
         #       test()
